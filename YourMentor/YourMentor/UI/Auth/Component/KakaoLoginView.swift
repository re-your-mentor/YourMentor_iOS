...
struct KakaoLoginView: UIViewRepresentable {
    ...
    class Coordinator: NSObject, WKNavigationDelegate {
        ...
        private func requestAccessToken(authCode: String) {
            ...
            let body = "grant_type=authorization_code&client_id=85c1c53c84f90c4bf17328e9561b3c40&redirect_uri=http://3.148.49.139:8000/auth/kakao/callback&code=\(authCode)"
            ...
            URLSession.shared.dataTask(with: request) { data, response, error in
                ...
                if let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any] {
                    print("✅ 응답 JSON: \(json)") // 응답 확인
                    
                    if let accessToken = json["access_token"] as? String {
                        print("✅ Access Token: \(accessToken)")
                        DispatchQueue.main.async {
                            self.parent.isLoginSuccess = true
                            self.parent.userId = 123456 // 실제 사용자 ID로 변경 필요
                        }
                    } else {
